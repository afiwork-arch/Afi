...
- ビルド後は必ずローカルで `npx wrangler dev --local` を立てて主要ページ（`/`, `/server/`,
  `/vpn/`, `/articles/`, `reviews/*`等）が200を返すか確認してから完了報告する。
  `python -m http.server` では拡張子なしURLの解決（`articles/foo` → `articles/foo.html`）が
  再現できず誤って404と判定してしまうため使わないこと（詳細は下記「デプロイの自動化」参照）。
...
新しいジャンル（サーバー以外）への展開手順は、下記「トップページ（ハブページ）とジャンルの
URL構成」の「新ジャンルを追加する手順」を参照（`columns.yaml`をコピーするのではなく、
同じシート内で`genres`リストにエントリを追加する設計）。
...
- **note**: 規約上アフィリエイトリンクを全面禁止してはおらず、「PR」等の開示表記があれば認められる
  運用（サバナビ本体のPR開示表記と同じステマ規制対応の発想でそのまま運用できる）。読者層も一般消費者
...
- `/`（`templates/home.html`）が**ジャンル横断のハブページ**。2026-09-15の「サバナビ2026」
  デザイン刷新でBentoグリッド構成になった（ジャンルタイル・統計タイル・記事カルーセル。
  絵文字アイコンは廃止済み。詳細は下記「サバナビ2026デザイン刷新」参照）。記事は
  `articles`全件をカルーセル表示（日付降順、手動更新不要）。
...
- **新ジャンルを追加する手順**:
  1. `templates/index.html` の `genre_key` 分岐に、そのジャンル向けの `{% elif %}` 分岐を足す
     ...
     文言がそのまま出てしまう。
  2. `templates/style.css` に新ジャンル用の識別色ルール `.tile-genre.genre-{key} { background:
     ...; color: ...; }` を追加する（無くても見た目が崩れるだけでビルドは通る。既存は
     `server`=`--primary`、`vpn`=`--accent`を使用。詳細は「サバナビ2026デザイン刷新」参照）。
...
- ステマ規制（景品表示法、2023年10月施行）対応の開示は、2026-09-15時点でフッター（全ページ
  共通、`base.html`）に一本化している（以前はヘッダー直下に`.pr-bar`も常設していたが、
  「AI感を減らす」節に記載の経緯でユーザー判断により廃止し削除済み）。「本サイトはPR
  (アフィリエイト広告)を含みます...」という文言と`disclaimer`ページへの導線がフッターにある。
