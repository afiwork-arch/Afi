...
- **ボタン群は役割ごとに見た目を変える**。同じpage内に複数のボタングループが並ぶと同じ見た目では
  ...
  - `.genre-card`（トップページのジャンル選択、`templates/home.html`）: タイトル＋件数＋矢印を
    横並びにした大きめのカード（2026-09-15にアイコン絵文字は削除、下記「AI感を減らす」参照）。
    ホバーで軽く浮き上がる（枠線色の変化＋控えめなbox-shadow）
  - `.pickup-article-card`（トップページの注目記事、`templates/home.html`）: タイトル＋日付の
    縦積みカード。`.more-articles-grid a`（各ジャンルページの記事リンク）より一回り大きく、
    タイトルは`-webkit-line-clamp`で2行に丸める
...
- **「引き算」だけでは単調になりすぎた反省（2026-09-15）**: 上記の引き算対応後、ユーザーから
  ...
  **踏んだ罠**: `.hero`をpaddingありのボックスにした際、中の`h1`/`p`がブラウザ既定の
  ...
  paddingを持つボックスに見出し・段落をそのまま入れる設計（`.diagnosis`, `.diag-result-card`
  等）を新しく作る際は、このパターンを毎回疑うこと（既存の`.article-body :first-child`も同じ
  対策）。
...
- **稼働中のジャンル**: `server`（レンタルサーバー/VPS、`/server/`）、`vpn`（VPN、`/vpn/`。
  ...
  診断ツール(serverのみ)/tier-toolbar(serverのみ)/比較表のスペック列・無料SSL列(serverのみ)/
